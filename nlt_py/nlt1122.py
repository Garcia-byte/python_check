...
while k < 20:
    ...
    # ECIET = ECI * E.T;
    #舒尔消元结果
    S = (B - E * C_inverse * E.T).todense()  #.todense转化为np.mat类型
    X = S.shape#.todense将稀疏矩阵转为稠密矩阵
    V = (-gc + E * C_inverse * gp).todense()

    b = (-gc + E * C_inverse * gp)
    delta_pc = np.linalg.solve(S, X)     #这里求解delta_pc直接用函数，但也可以用cholesky求解（硬件）
    #输入都是dence的格式，输出也还是dence格式
    ...
    pc_new = pc + Delta_pc.backward(d_pc, pc, 0.9)
    pp_new = pp + Delta_pp.backward(d_pp, pp, 0.9)
