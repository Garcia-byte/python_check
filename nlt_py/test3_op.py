...
class Adam():
    ...
    #adagrad
    def backward2(self, g, WB, lr):
        self.vt = self.vt + (g ** 2)
        vtt_w = self.vt + self.e
        vtt_sqrt_w = np.sqrt(vtt_w)

        NWB = WB + (lr * g / vtt_sqrt_w)
        return NWB
...
while k < 20:
    ...
    #     break
    if norm_dp <= (1e-100000000000) * norm_p:
        break
    else:
        #原算法直接在pc，pp上加delta，现在我们先求出delta，对其进行adam，再加在pc上
        d_pc = np.zeros(n_cameras * 9).reshape((n_cameras, -1))  #array？16*9
        d_pp = np.zeros(n_points * 3).reshape((n_points, -1))
        pc = camera_params
        pp = points_3d
        # pc_new = camera_params
        # pp_new = points_3d
        for i in range(n_cameras):
            d_pc[i, 0] += delta_pc[9 * i + 0, 0]  # 0
        ...
        Delta_pc = Adam(np.zeros((50, 9)))
        Delta_pp = Adam(np.zeros((20431, 3)))

        pc_new = Delta_pc.backward2(d_pc, pc, 0.1)
        pp_new = Delta_pp.backward2(d_pp, pp, 0.1)

        e_new = calc_error(camera_indices, point_indices, pc_new, pp_new).todense()
        # print(e_new)
        if np.linalg.norm(error-e_new) ** 2 < 1.0e-17999:    #一个csc格式，一个dense格式
            break
#######
        n1 = np.linalg.norm(error.todense()) ** 2
        n2 = np.linalg.norm(e_new) ** 2    #加了todense
...
######
        if rho[0, 0] > 0:
            ...
            if np.linalg.norm(g, ord=np.Inf) <= (1e-36):
                break
