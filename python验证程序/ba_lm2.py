...
def bundle_adjustment_sparsity(camera_indices, point_indices, camera_params, points_3d):
    ...
    # 标记已知的非0元素
    JC = lil_matrix((m, 6 * n_cameras), dtype=float)
    ...
    for s in range(camera_indices.size):
        ...
        vf, vk0, vk1, ux, uy, uz, vx, vy, vz = \
            ja.calcaJ(camera_sli, point_sli, False)
        ...
        JC[s * 2, index_c * 6 + 0] = uw0  # uw0
        JC[s * 2, index_c * 6 + 1] = uw1  # uw1
        JC[s * 2, index_c * 6 + 2] = uw2  # uw2
        JC[s * 2, index_c * 6 + 3] = ut0  # ut0
        JC[s * 2, index_c * 6 + 4] = ut1  # ut1
        JC[s * 2, index_c * 6 + 5] = ut2  # ut2
        # JC[s * 2, index_c * 6 + 6] = uf  # uf
        # JC[s * 2, index_c * 6 + 7] = uk0  # uk0
        # JC[s * 2, index_c * 6 + 8] = uk1  # uk1
        JC[s * 2 + 1, index_c * 6 + 0] = vw0  # vw0
        JC[s * 2 + 1, index_c * 6 + 1] = vw1  # vw1
        JC[s * 2 + 1, index_c * 6 + 2] = vw2  # vw2
        JC[s * 2 + 1, index_c * 6 + 3] = vt0  # vt0
        JC[s * 2 + 1, index_c * 6 + 4] = vt1  # vt1
        JC[s * 2 + 1, index_c * 6 + 5] = vt2  # vt2
        # JC[s * 2 + 1, index_c * 6 + 6] = vf  # vf
        # JC[s * 2 + 1, index_c * 6 + 7] = vk0  # vk0
        # JC[s * 2 + 1, index_c * 6 + 8] = vk1  # vk1
...
IC = dia_matrix((AC.diagonal(), [0]), shape=(6 * n_cameras, 6 * n_cameras)).tocsc()
...
while k < 10:
    ...
    B = AC + mu * IC  # identity(6 * n_cameras) #IC
    E = JC.T * JP
    ...
    #                                   camera_params.reshape((6 * n_cameras, -1))))):
    #     break
    if norm_dp <= (1e-16) * norm_p:
        ...
        for i in range(n_cameras):
            pc_new[i, 0] += delta_pc[6 * i + 0, 0]  # 0
            pc_new[i, 1] += delta_pc[6 * i + 1, 0]  # 1
            pc_new[i, 2] += delta_pc[6 * i + 2, 0]  # 2
            pc_new[i, 3] += delta_pc[6 * i + 3, 0]  # 3
            pc_new[i, 4] += delta_pc[6 * i + 4, 0]  # 4
            pc_new[i, 5] += delta_pc[6 * i + 5, 0]  # 5
            # pc_new[i, 6] += delta_pc[6 * i + 6, 0]  # 6
            # pc_new[i, 7] += delta_pc[6 * i + 7, 0]  # 7
            # pc_new[i, 8] += delta_pc[6 * i + 8, 0]  # 8
        ...
        # if np.linalg.norm(error-e_new) ** 2 < 1.0e-10:
        #     break
