...
def bundle_adjustment_sparsity(camera_indices, point_indices, camera_params, points_3d):
    ...
    for s in range(camera_indices.size):
        ...
        JC[s * 2, index_c * 9 + 0] = uf  # uw0
        JC[s * 2, index_c * 9 + 1] = uk0 # uw1
        JC[s * 2, index_c * 9 + 2] = uk1 # uw2
        JC[s * 2, index_c * 9 + 3] = uw0 # ut0
        JC[s * 2, index_c * 9 + 4] = uw1 # ut1
        JC[s * 2, index_c * 9 + 5] = uw2 # ut2
        JC[s * 2, index_c * 9 + 6] = ut0 # uf
        JC[s * 2, index_c * 9 + 7] = ut1 # uk0
        JC[s * 2, index_c * 9 + 8] = ut2 # uk1
        JC[s * 2 + 1, index_c * 9 + 0] = vf  #vw0
        JC[s * 2 + 1, index_c * 9 + 1] = vk0 #vw1
        JC[s * 2 + 1, index_c * 9 + 2] = vk1 #vw2
        JC[s * 2 + 1, index_c * 9 + 3] = vw0 #vt0
        JC[s * 2 + 1, index_c * 9 + 4] = vw1 #vt1
        JC[s * 2 + 1, index_c * 9 + 5] = vw2 #vt2
        JC[s * 2 + 1, index_c * 9 + 6] = vt0 #vf
        JC[s * 2 + 1, index_c * 9 + 7] = vt1 #vk0
        JC[s * 2 + 1, index_c * 9 + 8] = vt2 #vk1
...
error = calc_error(camera_indices, point_indices, camera_params, points_3d).tocsc()
...
while k < 10:
    ...
    # delta_pp = (C_inverse * -JP.T * E).todense()
    # # delta_pp = spsolve(JP.T * JP, -JP.T * E)
    ...
    for i in range(n_cameras):
        camera_params[i, 0] += delta_pc[9 * i + 3, 0]
        camera_params[i, 1] += delta_pc[9 * i + 4, 0]
        camera_params[i, 2] += delta_pc[9 * i + 5, 0]
        camera_params[i, 3] += delta_pc[9 * i + 6, 0]
        camera_params[i, 4] += delta_pc[9 * i + 7, 0]
        camera_params[i, 5] += delta_pc[9 * i + 8, 0]
        camera_params[i, 6] += delta_pc[9 * i + 0, 0]
        camera_params[i, 7] += delta_pc[9 * i + 1, 0]
        camera_params[i, 8] += delta_pc[9 * i + 2, 0]
    ...
    norm = np.linalg.norm(delta_pp)**2 + np.linalg.norm(delta_pc)**2
